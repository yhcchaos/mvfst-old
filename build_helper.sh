...
function setup_folly() {
  ...
  if [ ! -d "$FOLLY_DIR" ] ; then
    ...
    git clone --branch v2019.10.07.00 https://github.com/facebook/folly.git "$FOLLY_DIR"
...
function setup_fizz() {
  ...
  if [ ! -d "$FIZZ_DIR" ] ; then
    ...
    git clone --branch v2019.10.07.00 https://github.com/facebookincubator/fizz "$FIZZ_DIR"
  fi
